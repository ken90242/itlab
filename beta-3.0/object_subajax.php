	...
	if($query_run =  mysql_query($query)){
		...
		for($i=2;$i<sizeof($sub_c_arr);$i=$i+2){
			if (strpos($sub_c_arr[$i],$e_class) !== false) {
			    ...
			    echo $e_class.';'.$e_id.';'.$e_name.';'.'<input type="hidden" name="equ_id'.$_SESSION["times"].'" value="'.$e_id.'"><input type="hidden" name="usr_id'.$_SESSION["times"].'" value="'.$u_id.'"><input type="hidden" name="equ_stats'.$_SESSION["times"].'" value="'.$e_stus.'"><input type="hidden" name="equ_cls'.$_SESSION["times"].'" value="'.$e_class.'"><input type="hidden" name="inpot'.$_SESSION["times"].'" value="物品名稱 : '.$e_name.'<br>物品類別 : '.$e_class.'"><input type="hidden" class="equ_add_clas" name="equ_add_clas'.$_SESSION["times"].'"><input type="hidden" class="sub_parent" name="sub_parent'.$_SESSION["times"].'" value="'.$sub_c_arr[$i-1].'">;';
			    // echo $sub_c_arr[$i-1];
			    $token=1;
			}
		}
