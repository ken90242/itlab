<?php
require '../connect_database_latin1.php';
if ($_POST) {
	$search = trim($_POST["record_name_search"]);
	$NewString_search = preg_split("/[\s,]+/",$search);
	if(count($NewString_search)<=1){
		$sql = 'SELECT * FROM `trade_analyze` WHERE `u_id` LIKE \'%'.$NewString_search[0].'%\' OR `e_id` LIKE \'%'.$NewString_search[0].'%\' OR `time` LIKE \'%'.$NewString_search[0].'%\' OR `deadline_time` LIKE \'%'.$NewString_search[0].'%\' OR `return_time` LIKE \'%'.$NewString_search[0].'%\' OR `e_addition_class` LIKE \'%'.$NewString_search[0].'%\'';
	}
